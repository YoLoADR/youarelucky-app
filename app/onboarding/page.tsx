...
import MultiStepForm from '@/components/wordCloud';
import Carousel from '@/components/plugins';
...
import { demoOtherQuestions } from '@/variables/demoQuestion';
...
export default function Onboarding() {
  ...
  const handleDemo = (demoNumber: number) => {
    ...
    setOutputCode('');  // Réinitialiser la sortie pour éviter les doublons
    setLoading(true);  // Activer l'indicateur de chargement
  
    // Générer un délai aléatoire entre 3000 et 4000 ms
    ...
    setTimeout(() => {
      ...
      // Fonction pour ajouter un caractère à la fois avec gestion correcte de l'état
      const typeCharacter = () => {
        if (index < message.length) {
          setOutputCode((prev) => prev + message.charAt(index - 1));  // Utiliser charAt pour éviter les erreurs d'index
          index++;
          setTimeout(typeCharacter, 30); // Délai plus rapide pour simuler le streaming
        }
      };
  
      typeCharacter();  // Commencer à ajouter des caractères un à un

      // setOutputCode(demoQuestions[demoNumber - 1].answer);
    }, randomDelay);
  ...
  const renderSelectedPlugin = () => {
    switch (selectedPlugin) {
      ...
      case 2:
        return <QuestionSuggestions demoQuestions={demoQuestions} handleDemo={handleDemo} />;
      case 3:
        return <QuestionSuggestions demoQuestions={demoOtherQuestions} handleDemo={handleDemo} />;
      default:
        return null;
    }
  };
          ...
          >
            <Flex
              ...
              onClick={() => {
                ...
                setShowAccordion(false); // Masquer l'accordéon
              }}
            >
              <Flex
                borderRadius="full"
            ...
            <Flex
              ...
              onClick={() => {
                ...
                setShowAccordion(true); // Afficher l'accordéon
              }}
            >
              <Flex
                borderRadius="full"
            ...
            <>
              ...
              >
                <AccordionItem border="none">
                  ...
                  <AccordionPanel mx="auto" p="0px 0px 10px 0px">
                    <Carousel onSelectPlugin={setSelectedPlugin} />
